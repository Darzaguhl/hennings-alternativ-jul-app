...
export default function VakterSection({ eventId, isAdmin }: { eventId: number; isAdmin: boolean }) {
  ...
  const loadShifts = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(`Unable to load vakter (${response.status})`);
      const data: Shift[] = await response.json();
      setShifts(data);
    } catch (err) {
      console.error("Error loading shifts", err);
      Alert.alert("Error", "Could not load vakter right now.");
    } finally {
      setLoading(false);
    }
  }, [apiFetch, eventId]);
  ...
  const handleCreateShift = useCallback(async () => {
    ...
    if (!trimmedTitle) {
      Alert.alert("Missing title", "Please enter a title for this vakt.");
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(body?.detail ?? "Unable to create vakt.");
      }
      ...
      Alert.alert("Error", err?.message ?? "Failed to create vakt.");
      setCreating(false);
    }
  ...
  const submitSignup = useCallback(
    async (shift: Shift, experience?: { has_relevant_experience: boolean; experience_notes: string }) => {
      ...
      try {
        ...
        const body = await response.json().catch(() => ({}));
        if (!response.ok) {
          throw new Error(body?.detail ?? "Unable to sign up for this vakt.");
        }
        await loadShifts();
      } catch (err: any) {
  ...
  return (
    <View style={styles.section}>
      <View style={styles.sectionHeader}>
        <Text style={styles.sectionTitle}>Vakter</Text>
        {isAdmin && (
          <TouchableOpacity style={styles.addButton} onPress={() => setShowCreateModal(true)}>
            <Text style={styles.addButtonText}>+ Add vakt</Text>
          </TouchableOpacity>
        )}
      </View>
      ...
      {loading ? (
        ...
        <Text style={styles.placeholder}>No vakter added for this event yet.</Text>
      )}
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <Text style={styles.modalBody}>
              This is a critical vakt. Do you have relevant previous experience or education?
            </Text>
      ...
      <Modal transparent animationType="slide" visible={showCreateModal} onRequestClose={closeCreateModal}>
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <Text style={styles.modalTitle}>Add Vakt</Text>
